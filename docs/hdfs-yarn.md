...
The file `datanodes` is used by startup scripts to start required daemons on all nodes. Edit `~/hadoop/etc/hadoop/datanodes` to include both of the nodes:

{{< file "~/hadoop/etc/hadoop/datanodes" resource >}}
node1
node2
...
1.  Start the HDFS by running the following script from **node-master**:
    ...
    This will start **NameNode** and **SecondaryNameNode** on node-master, and **DataNode** on **node1** and **node2**, according to the configuration in the `datanodes` config file.
